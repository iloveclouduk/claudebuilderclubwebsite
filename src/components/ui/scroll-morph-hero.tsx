...
export default function IntroAnimation({ scrollContainerRef }: IntroAnimationProps) {
    ...
    // Map scroll progress (0-1) to our virtual scroll range (0-3000)
    ...
    // --- Content Opacity ---
    // Fade in content when arc is formed (morphValue > 0.4)
    const contentOpacity = useTransform(smoothMorph, [0.4, 0.7], [0, 1]);
    const contentY = useTransform(smoothMorph, [0.4, 0.7], [20, 0]);

    return (
        <div ref={containerRef} className="relative w-full h-full bg-white overflow-hidden" onMouseMove={handleMouseMove}>
            {/* Container */}
